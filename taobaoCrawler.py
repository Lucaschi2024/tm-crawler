...
CONST_CHROMEDRIVER_PATH = f.readline().strip()
f.close()
...
service = webdriver.ChromeService(executable_path=CONST_CHROMEDRIVER_PATH)
    ...
    try:
        if tbPageVersion == 0:
            ...
            for i, goods in enumerate(goods_arr):
                ...
                shop_link = goods.find_element(By.CSS_SELECTOR,
                                               'div[class^="ShopInfo--wwWrap"] > span > a').get_attribute(
                    'href')
                item_link = goods.find_element(By.CSS_SELECTOR,
                                               'a[class^="Card--doubleCardWrapper"]').get_attribute(
                    'href')
                goods_item = {"商品名称": item_name, "商品价格": item_price, "月销售量": month_deals,
